$(document).ready(function () {
    $(document).on("click", ".select-sales-person", function () {
        var value = this.value;
        var that = $("#salesPerson-" + value).closest("tr");
        var fullName = that.find("td").eq("2").text();
        var statusValue;
        switch (that.find("td").eq(6).text()) {
            case "Active":
                statusValue = "AV";
                break;
            case "Deleted":
        ...
        $("#salesPersonId")[0].value = that.find("td").eq(1).text();
        ...
        $("#itemLimit")[0].value = that.find("td").eq(7).text();
        $("#status option[value='" + statusValue + "']").attr("selected", "selected");
        $("#description")[0].value = that.find("input").eq(1).val();
    });

    $("#salesPersonId").on("change", function () {
        getSalesPersonInfo($(this));
    });

    $("#basic-addon1").on("click", function () {
        $("#salesPersonId").removeAttr("readonly");
    });

    $("#sales-person-search").on("click", function () {
        var inputElement = $(".input-form input");
        var valueArr = inputElement.map(function (index, input) {
            return input.value;
        });
        location.href = "/salesPerson/search?salesPersonId=" + valueArr[0] + "&firstName=" + valueArr[1] + "&lastName="
            + valueArr[2] + "&address=" + valueArr[3] + "&email=" + valueArr[4] + "&phone=" + valueArr[5] +
            "&itemLimit=" + valueArr[6] + "&status=" + $("#status")[0].value;
    });
    ...
    $(document).on("click", ".popover-footer .btn-delete", function () {
        var salesPersonId = $("#salesPersonId")[0].value;
        location.href = "/salesPerson/" + salesPersonId;
    });
});